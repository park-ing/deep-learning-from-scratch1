...
class TwoLayerNet:
    def __init__(self, input_size, hidden_size, output_size, weight_init_std=0.01):
        ...
        # np.random.randn(a, b) -> (a,b)크기 행렬로 정규분포로 초기화. 랜덤한 숫자로 생성
        self.params['b1'] = np.zeros(hidden_size) # 이 예시에는 0으로 초기화
    ...
    # x : 입력 데이터, t : 정답 레이블
    def numerical_gradient_mul(self, x, t):
        loss_W = lambda W: self.loss(x, t)

        grads = {}
        grads['W1'] = numerical_gradient_mul(loss_W, self.params['W1'])
        grads['b1'] = numerical_gradient_mul(loss_W, self.params['b1'])
        grads['W2'] = numerical_gradient_mul(loss_W, self.params['W2'])
        grads['b2'] = numerical_gradient_mul(loss_W, self.params['b2'])

        return grads

# mnist 데이터셋을 사용하여 학습 수행
####****** 지금은 수치미분방식이라 시간이 엄청 오래 걸린다.
from dataset.mnist import load_mnist
...
for i in range(iters_num):
    ...
    # 기울기 계산
    grad = network.numerical_gradient_mul(x_batch, t_batch)
